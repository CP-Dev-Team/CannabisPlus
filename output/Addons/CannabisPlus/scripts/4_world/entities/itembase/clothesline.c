class Clothesline extends ShelterSite
{
	const float CP_MAX_ACTION_DETECTION_ANGLE_RAD 		= 1.3;		//1.3 RAD = ~75 DEG
	const float CP_MAX_ACTION_DETECTION_DISTANCE 		= 2.0;		//meters
	...
	override bool IsFacingCamera( string selection )
	{
		...
		if ( ref_dir.Length() != 0 )
		{
			...
			if ( angle >= CP_MAX_ACTION_DETECTION_ANGLE_RAD )
			{
				return true;
			}
		}
	...
	override bool HasProperDistance( string selection, PlayerBase player )
	{
		if ( MemoryPointExists( selection ) )
		{
			...
			if ( distance >= CP_MAX_ACTION_DETECTION_DISTANCE )
			{
				return false;
			}
		}
	...
	override void SetActions()
	{
		...
		AddAction(ActionBuildClothesline);
	}
}
