class CP_WorkBenchCraftKit extends RecipeBase
{	
	override void Init()
	{
		m_Name = "Make Cannabis Workbench Kit";
		...
		//result1
		AddResult("CP_Workbench_Kit");//add results here
...
{	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"CP_Workbench");//you can insert multiple ingredients this way
		...
		m_ResultSetQuantity[1] = 15;//-1 = do nothing
	...
	override bool CanDo(ItemBase ingredients[], PlayerBase player)//final check for recipe's validity
	{
		...
		if(ingredient0.GetInventory().AttachmentCount() == 0)
		{
			return true;		
		}
		return false;
